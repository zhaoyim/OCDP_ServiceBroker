...
public class ambariClient {
	...
	public String getCapacitySchedulerConfig(String rmHost) {
		...
		URI uri = buildUri("api/v1/clusters/" + this.clusterName, "",
				"configurations?type=capacity-scheduler&tag=" + versionTag);
	...
	public String refreshYarnQueue(String rmHost) {
		...
		String refreshRequestEntity = buildRequestEntity("YARN", rmHost, "RESOURCEMANAGER", "capacity-scheduler",
				"Refresh YARN Capacity Scheduler", "REFRESHQUEUES");
	 ...
	 * @return
	 */
	private String buildRequestEntity(String serviceName, String hosts, String componentName, String configTags,
			String context, String command) {
		String requestEntity = null;

		requestEntity = "{\"Requests\\/resource_filters\":" + "[{\"service_name\":" + "\"" + serviceName + "\""
				+ ",\"hosts\":" + "\"" + hosts + "\"" + ",\"component_name\":" + "\"" + componentName + "\"" + "}],"
				+ "\"RequestInfo\":" + "{\"parameters\\/forceRefreshConfigTags\":" + "\"" + configTags + "\""
	...
	private String executeRequest(HttpUriRequest request) {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				logger.error("HTTP response code not 200: " + response.getStatusLine().getStatusCode() + "="
						+ response.getStatusLine().getReasonPhrase());
				logger.error("HTTP response failed msg: " + response.getEntity());
			}
			response.close();
		} catch (IOException e) {
