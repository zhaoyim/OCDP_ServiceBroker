...
import com.asiainfo.cm.servicebroker.dp.model.CapacitySchedulerConfig;
...
public class YarnCapacityCalculator {
	...
	public String applyQueue(Long quota, String queueName) {
		// To make sure that the sum of all the queues capacity equals 100%, the patch
		// for ambari server to support
		...
		String resourcePoolCapacity = String.format("%.2f",
				(availableCapacity - (Double.parseDouble(targetQueueCapacity))));
	...
	public void updateQueue(String queueName, Long quota) {
		...
		String originQueueCapacity = properties.get("yarn.scheduler.capacity." + queueName + ".capacity");
		String targetQueueCapacity = String.format("%.2f", ((100 * quota) / (totalMemory / 1024)));
		String resourcePoolCapacity = String.format("%.2f", (availableCapacity
				- (Double.parseDouble(targetQueueCapacity) - Double.parseDouble(originQueueCapacity))));
		// Update queue
	...
	public boolean revokeQueue(String serviceInstanceResuorceName) {
		...
		String targetQueueCapacity = properties
				.get("yarn.scheduler.capacity." + serviceInstanceResuorceName + ".capacity");
	...
	private void resetQueueCapacity(String queueName, String oldQueueCapacity, String targetQueueCapacity) {
		if (oldQueueCapacity == null) {
			...
			properties.replace("yarn.scheduler.capacity." + queueName + ".capacity", oldQueueCapacity,
					targetQueueCapacity);
			properties.replace("yarn.scheduler.capacity." + queueName + ".maximum-capacity", oldQueueCapacity,
					targetQueueCapacity);
		}
	}
